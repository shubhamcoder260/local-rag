    ...
    for word in words:
        ...
        if current_length +len(word) +1>max_chars:
            ...
            for w in reversed(current_chunk):
                # FIX (v2): check overlap_len + len(w) BEFORE adding w, not
                # overlap_len - len(w). The original subtraction let the
                # accumulated overlap overshoot overlap_chars by a wide
                # margin before the break condition ever tripped - proven
                # with real data during Phase 2 benchmarking (measured
                # overlap consistently ran 5-30 chars over the configured
                # target across every tested document).
